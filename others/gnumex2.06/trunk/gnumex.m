...
% mex bat options file creator for Cygwin or MinGW GCC
%
% gnumex(action)
%   possible actions:
%         <nothing>         Opens the Gnumex dialog
%         'version'         Returns the version of Gnumex
%         'about'           Displays information about Gnumex
%         'startup'         Opens the Gnumex dialog
%         'cfg_defaults'    
%         'defaults'
%         'struct2fig'
%         'fig2struct'
...
switch action
    ...
    case 'startup'
        ...
        cygfig = figure('color', get(0,'defaultuicontrolbackground'),...
    ...
    case 'parsepaths'
        ...
        for i=1:length(fnames)
            if ~isempty(findstr(fnames{i},'path'))
                ...
                s = adddriveletter(s);
                if checkMatlabMinVersion('7.6')
                    if ~isempty(s)
                        s = getwinshortpath(s);
                    end
                elseif checkMatlabMinVersion('7.4')
                    s = shortpath74(s);
                else
                    s = shortpath(s);
                end
    ...
    case 'makeopt'
        ...
        % get matlab version
        % find perl location
        if ~checkMatlabMinVersion('6')  % matlab 5
        ...
        if checkMatlabMinVersion('7')
            ...
            if pps.mexf == 1 % mex file
                ...
                librootn = 'mexlib';
                if checkMatlabMinVersion('7'), libraries = mexlibs; end
            else % engine file
                if pps.lang == C  % c/c++
        ...
        if pps.lang ~= C % fortran
            ...
            if ~checkMatlabMinVersion('7.4'), c = ['-mrtd ' c]; end
    ...
    otherwise % otherwise unrecognized option
...
end %end function adddriveletter
...
end %end function correct_mex_pl


%% checkMatlabMinVersion
function mlv_ge = checkMatlabMinVersion(v)
    % checks that Matlab version is at least version v. Returns true if greater
    % than or equal to v.  False otherwise.
...
end %end function checkMatlabMinVersion