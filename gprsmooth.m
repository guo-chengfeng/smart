function pdat = gprsmooth(cdat,varargin)
%GPRSMOOTH smmoth the data along X and Z axis.
%   gprsmooth smooth the data along X axis using the past data point 
%                and the data along Z axis using the past and future data.
%   Usage:
%   pdat=gprsmooth(cdat)           % smStep: 4, dirflag='z';
%   pdat=gprsmooth(cdat,smStep)    % dirflag='z';
%   pdat=gprsmooth(cdat,dirflag)   % smStep: 4;
%   pdat=gprsmooth(cdat,smStep,dirflag)
%   pdat=gprsmooth(cdat,dirflag,smStep)
%   
%   Author: C.F. Guo
%   Date:   26-Oct-2016
